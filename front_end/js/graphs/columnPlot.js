...
export class ColumnPlot extends Graph {
    ...
    updateGraph(svg) {
        ...
        console.log(xSelectedField)

        d3.selectAll(".columnAxis")
            .remove()

        let values = this.movies.map(movie => movie[xSelectedField])
        ...
        const genreByYear = [...new Set(this.movies.map(x => x[xSelectedField]).sort())].map(selElement => {
            let obj = {
        ...
        console.log(genreByYear)

        const maxValue = genreByYear.map(x => x.sum).reduce((prev, current) => Math.max(prev, current), -Infinity)
        ...
        let a = d3.stack().keys(genres.map(x => x.name))(genreByYear)
        ...
        svg.append("g")
            ...
            .data(d => d)
            .enter().append("rect")
            .attr("x", d => this.xScaleLinear(d.data.selElement))// d=>this.xScaleLinear(d[1]))
            .attr("y", d => y(d[1]))
